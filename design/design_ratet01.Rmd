...
```{r}
#| include: false
rice_session_open(username = "butcherb", password = Sys.getenv("ENTIMICE_PASSWORD"))

path_st2 <- "root/clinical_studies/RO7187807/CDT70155/GB39242/data_analysis/CSR/prod/outdata_vad/"

asl_st2 <- rice_read(paste0(path_st2, "asl.sas7bdat"), prolong = TRUE)
aef_st2 <- rice_read(paste0(path_st2, "aef.sas7bdat"), prolong = TRUE)

rice_session_close()

anl_st2 <- aef_st2 %>%
  filter(MITTFL == "Y") %>%
  filter(PARAMCD == "TNPMEXAB") %>%
  left_join(
    asl_st2 %>%
      select(USUBJID, TRT02P, TMATRSK, BEOSST, REGIONST, ICSDOSST, ASMEXGST),
    by = "USUBJID"
  ) %>%
  select(USUBJID, TRT02P, AVAL, TMATRSK, BEOSST, REGIONST, ICSDOSST, ASMEXGST) %>%
  mutate(
    TRT02P = str_remove(TRT02P, " MSTT1041A"),
    TRT02P = factor(TRT02P, levels = c("PLACEBO", "70 mg", "210 mg", "490 mg")),
    lnTMATRSK = log(TMATRSK),
    AVALF = factor(AVAL)
  )
```

```{r}
covars_st2 <- c("BEOSST", "REGIONST", "ICSDOSST", "ASMEXGST")

lyt_st2 <- basic_table() %>%
  split_cols_by("TRT02P", ref_group = "PLACEBO") %>%
  add_colcounts() %>%
  summarize_vars(
    "AVALF",
  ...
  summarize_glm_count(
    vars = "AVAL",
    variables = list(arm = "TRT02P", offset = "lnTMATRSK", covariates = NULL),
    conf_level = 0.95,
  ...
  summarize_glm_count(
    vars = "AVAL",
    variables = list(arm = "TRT02P", offset = "lnTMATRSK", covariates = covars_st2),
    conf_level = 0.95,
...
build_table(lyt = lyt_st2, df = anl_st2)
```

-   Matches SAS out to 4 decimals
